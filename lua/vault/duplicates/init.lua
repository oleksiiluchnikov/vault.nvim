...
function M.review(root, opts)
    ...
    open_current = function()
        ...
        end, vim.tbl_extend("force", keymap_opts, { desc = "Preview keep B result" }))
        vim.keymap.set("n", "p", function()
            preview_merge_result(item.recommended)
        end, vim.tbl_extend("force", keymap_opts, { desc = "Preview recommended result" }))
        vim.keymap.set("n", "]c", function()
            jump_hunk(1)
        end, vim.tbl_extend("force", keymap_opts, { desc = "Next changed region" }))
        vim.keymap.set("n", "[c", function()
            jump_hunk(-1)
        end, vim.tbl_extend("force", keymap_opts, { desc = "Previous changed region" }))
        ...
        end, vim.tbl_extend("force", keymap_opts, { desc = "Skip duplicate" }))
        vim.keymap.set("n", "q", function()
            close_ui()
            save_session_state()
            log.info("Duplicate review paused at %d/%d", state.index, #state.items)
        end, vim.tbl_extend("force", keymap_opts, { desc = "Quit duplicate review" }))
        ...
        end, vim.tbl_extend("force", keymap_opts, { desc = "Next duplicate" }))
